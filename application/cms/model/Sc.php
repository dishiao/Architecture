...
class Sc extends ThinkModel
{
    ...
    public static function getList($map = [], $order = [])
    {
        ...
        if (isset($_GET['sc_cate']) && $_GET['sc_cate']!='0'){
            $get_sc_cate = $_GET['sc_cate'];
            $map['is_insc.name'] = $get_sc_cate;
        }//是否入川 1 入川 0川内
        if (isset($_GET['is_sc'])){
            $get_is_sc = $_GET['is_sc'];
            $map['is_insc.is_sc'] = $get_is_sc;
        }
            ...
            for ($i=0;$i<$cids_num;$i++){
                ...
                $data_list[$i] = Db::view('query_project','project_num,project_pnum,project_area,project_unit,project_unitnum,project_type,project_nature,project_use,project_allmoney,project_acreage,project_level,project_wnum')
                    ...
                    ->view('sichuan_company',
                        'company_name,company_num,company_person,company_addr,company_reg,company_chid,add_time',
                        'sichuan_company.company_num = query_company.company_num',
                        'LEFT'
                    )
                    //四川资质
                    ->view('sc_zhizi',
                        'cnum,term,grade,range,manager,leader,aptitude',
                        'sichuan_company.id = sc_zhizi.cid',
                        'LEFT'
                    )
                    //四川公司信息
                    ->view('is_insc',
                    ...
                    ->view('query_pfinish',
                        ...
                        'LEFT')
                    ->view('query_rank',
                        'rank_build_house,rank_build_gov,rank_survey_house,rank_survey_gov',
                        'query_company.company_url = query_rank.rank_url',
                        'LEFT')
                    ->where($map)
    ...
    public static function getListAll($getInfo){
        ...
        if (isset($getInfo['sc_cate']) && $getInfo['sc_cate']!='0'){
            $get_sc_cate = $getInfo['sc_cate'];
            $map['is_insc.name'] = $get_sc_cate;
        }//是否入川 1 入川 0川内
        if (isset($getInfo['is_sc'])){
            $get_is_sc = $getInfo['is_sc'];
            $map['is_insc.is_sc'] = $get_is_sc;
        }
    ...
    // 获取四川分类
     public static function get_sc_cate(){
         $sc_cate = Db::table('dp_is_insc')
             ->where(['name'=>['neq','']])
             ->distinct('name')
             ->column('name');
         return $sc_cate;
